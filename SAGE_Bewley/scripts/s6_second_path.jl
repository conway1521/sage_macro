# STAGE6.md Part 6, check 10: every taste node solved directly at its own
...
for c in CELLS
    ...
    ds = pmap(j -> begin
            s = solve_participation_logit(update(p0; social_strength = κ * ms[j] * c.B * arg), 1.0; theta = THETA, full = true)
            cell_summary(p0, s; thresholds = [(ypov, abar)])
        end, 1:NDIRECT)
