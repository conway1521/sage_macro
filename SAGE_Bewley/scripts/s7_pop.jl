# Population machinery for stage 7: stage 6 with a population of permanent
# discount-factor types behind every cell. Requires s7_common.jl and constants
# OMEGA and NABLA defined by the includer.
#
# The types are mixed inside the family build (see `collapse`), so everything
# downstream, pooling, equilibrium, hardship, agency, is unchanged: the mixture
# of stationary distributions is itself stationary and every summary field is
# linear in it.
...
# =============================================================== families ==
cache7(na) = (d = joinpath(@__DIR__, @sprintf("cache_s7_theta%.4f_amax%.1f_pexp%.1f_ne%d_na%d_nb%d",
                                              THETA, GRID.a_max, GRID.pexp, NE, na, NBETA)); isdir(d) || mkpath(d); d)
const FAMS = Dict{Any,Any}()
r8(x) = round(x, digits = 8)
function family_u(α, δ; rr = RR, subsidy = 0.0, lumptax = 0.0, partcredit = 0.0,
                  thresholds = nothing, na = NA, nabla = NABLA)
    thr = thresholds === nothing ? (0.0, 0.0) : (r8(thresholds[1][1]), r8(thresholds[1][2]))   # key on the anchored pair; the rest derive from it
    key = (r8(α), r8(δ), r8(rr), r8(subsidy), r8(lumptax), r8(partcredit), thr..., r8(nabla), na)
    haskey(FAMS, key) && return FAMS[key]
    file = joinpath(cache7(na), "fam_" * join([replace(@sprintf("%.6f", k), "." => "p") for k in key[1:end-1]], "_") * ".jls")
    ...
    p0s = [cell_params_u(α; δ = δ, f = F_FIND, rr = rr, na = na, ne = NE, a_max = GRID.a_max,
                         ...
                         partcredit = partcredit, β = b) for b in bs]
    nodes = build_family_u(p0s, UGRID, THETA; weights = w, threaded = true, thresholds = thresholds)
    p0 = p0s[1]
    ...
    fam = (u = copy(UGRID), nodes = nodes,
           ...
           alpha = α, delta = δ, rr = rr, thresholds = thresholds, nabla = nabla, betas = bs,
           r = [n.rate for n in nodes], minc = [n.minc for n in nodes], pbase = [n.pbase for n in nodes])
    @printf("  [family] alpha %.3f delta %.4f rr %.2f sub %.2f tax %.5f credit %.2f nabla %.3f thr %s na %d  (%.1f min)\n",
            α, δ, rr, subsidy, lumptax, partcredit, nabla, thresholds === nothing ? "none" : "set", na, (time() - t0) / 60)
    flush(stdout)
    serialize(file, fam)
    FAMS[key] = fam
end
