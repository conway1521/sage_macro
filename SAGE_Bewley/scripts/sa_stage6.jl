# Stage 6 driver: the S+A dashboard with unemployment risk. Specification and
# predictions in STAGE6.md; nothing here departs from it.
#
...
#
# Writes sa_stage6[_omNNN].txt (log), sa_stage6_results[_omNNN].txt (key = value),
# sa_stage6_valley[_omNNN].txt (the calibration valley).

include(joinpath(@__DIR__, "s6_workers.jl"))
const OMEGA = length(ARGS) >= 1 ? parse(Float64, ARGS[1]) : 0.30
const TAG   = abs(OMEGA - 0.30) < 1e-9 ? "" : @sprintf("_om%03d", round(Int, 100 * OMEGA))
...
@printf("stage 6 | omega %.2f | workers %d | theta %.4f | grid a_max %.1f pexp %.1f na %d ne %d\n",
...
if E5 === nothing
    ...
    @printf("at (kappa, sigma) = (10.00, 0.510): rate %.4f (stage 5b: 0.3526), groups %.4f / %.4f\n",
            E5.r, E5.lo, E5.hi)
...
const KAPPA = fine.κ; const SIGMA = fine.σ
E0 = fine.e
...
open(joinpath(@__DIR__, "sa_stage6_valley" * TAG * ".txt"), "w") do io
...
println()
println("P5: asset poverty among the employed vs stage 5b 0.428, and unemployed vs employed:")
@printf("    employed %.4f (%s), unemployed %.4f (%s)\n", B.asset_E, B.asset_E < 0.428 ? "fell" : "did NOT fall",
        B.asset_U, B.asset_U > B.asset_E ? "higher" : "NOT higher")
println("P6: income poverty confined to the unemployed and in [0.02, 0.05]:")
@printf("    employed income-poor %.4f (%s), population %.4f (%s)\n", B.inc_E, B.inc_E == 0 ? "none" : "SOME",
        B.inc, 0.02 <= B.inc <= 0.05 ? "in range" : "OUT of range")
...
println()
let s = rows[2].pop, e = rows[3].pop, c = rows[4].pop, u = rows[5].pop
    println("P7  subsidy raises A and lowers S: ", (s.A > base.A && s.rate < base.rate) ? "holds" : "FAILS")
    @printf("    low-cell hardship change under the subsidy %+.4f (AGENCY.md: asset poverty fell 0.042)\n",
            s.hl.union - base.hl.union)
    println("P8  higher replacement rate lowers income poverty and raises A: ",
            (u.inc < base.inc && u.A > base.A) ? "holds" : "FAILS")
    @printf("    employed asset poverty %.4f -> %.4f (%s), participation %+.4f\n",
            base.asset_E, u.asset_E, u.asset_E > base.asset_E ? "rose as predicted" : "did NOT rise", u.rate - base.rate)
    println("P9  empowerment mostly mechanical: ", abs(RESULTS["dA_alpha_emp"]) > 0.8 * abs(RESULTS["dA_emp"]) ? "holds" : "FAILS")
    println("P10 credit small positive on A: ", (0 < c.A - base.A < 0.02) ? "holds" : "FAILS")
    @printf("    credit reaching the unemployed: their income change %+.5f\n", rows[4].Pl.ym_U - rows[1].Pl.ym_U)
end
