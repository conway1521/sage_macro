...
\begin{abstract}
\noindent I model participation in social life as a discrete choice with social interactions inside an incomplete-markets economy, in which agency, the capacity to convert effort into income, differs across education groups. The main result is a bound that turns ordinary participation statistics into a test for coordination traps. With lognormal belonging tastes, the slope of the aggregate participation map depends on the taste distribution only through its dispersion and the participation rates that distribution induces, so multiple equilibria require taste dispersion below a threshold computable from observed group rates and one preference share, with no structural estimation. The bound tightens as the participation gradient steepens: an observed education gradient is evidence against coordination traps rather than neutral with respect to them. At the French moments the bound is $0.45$ against the dispersion of $0.50$ needed to fit the gradient, so the calibrated economy sits about ten percent outside the coordination region, and the model's own numerical boundary near $0.41$ confirms the ordering. Three results follow. The education gradient in participation divides evenly between the taste for belonging and agency operating through wealth, the income effect beating the price of time, which is why participation rises with income although richer households face a higher opportunity cost of time. A budget-balanced make-work-pay subsidy has a direct effect of eight percentage points and an equilibrium effect of twenty-nine, from 37 percent participation to 8, so outside the multiplicity region the complementarity still amplifies policy by a factor of 3.6. And a participation tax credit, the analogue of France's charitable-donations deduction and the United Kingdom's Gift Aid, moves participation the other way at lower fiscal cost, so two policies from the same pro-work family have opposite social consequences and appear as mirror images on a GDP and a GDP-B ledger that single-index evaluation cannot separate.
\end{abstract}
...
Fourth, the contrast that makes the policy point usable. Raising the lower-education group's agency to the population mean, an empowerment policy in the SAGE sense, raises equilibrium participation from 37 to 51 percent, with both groups participating more. A participation tax credit, modelled at the real rebate rates of the United Kingdom's Gift Aid and France's charitable-donations deduction, does the same from the other direction and more cheaply: Gift Aid's 25 percent rebate lifts participation to 83 percent at a fifth of the work subsidy's fiscal cost. Because such credits are claimed unevenly, I add a take-up gradient, and the result runs against the natural worry. The credit remains equalising at every take-up rate I try: the ratio of higher- to lower-group participation falls from 1.79 at baseline to between 1.01 and 1.51, so imperfect take-up makes the instrument less equalising rather than regressive. Policies from the same pro-work family therefore have opposite effects on the social fabric: subsidising hours raises the price of time and unravels participation, while empowering people or paying them for the act of joining crowds it in. An evaluation framework that does not carry the social dimension cannot tell these policies apart on the margin where they differ most.
...
At the calibrated equilibrium the aggregate rate is $r = 0.371$, group rates are $0.265$ and $0.476$, and $\omega = 0.30$. The complementarity factor is $1.251$ and the density term $0.363$, so $\bar\sigma = 0.454$. The dispersion required to fit the observed gradient is $\sigma_m = 0.50$, which exceeds the bound. Multiplicity is ruled out, and by an economically small margin: the calibrated economy sits about ten percent outside the boundary measured in taste dispersion.

Because the proposition abstracts from within-group wealth heterogeneity, which smooths each group's response and so lowers the true slope, it delivers a necessary condition rather than a sufficient one. The model's own numbers confirm the direction. At the calibrated point the map's actual slope is $G'(r^*) = 0.81$ against the proposition's $0.91$, both below one. Scanning the full model directly, multiple stable equilibria appear for $\sigma_m \le 0.40$ and are gone by $\sigma_m = 0.42$, placing the true boundary near $0.41$, inside the analytical bound as it must be; and the $1/\sigma_m$ scaling of the proposition predicts that boundary to two digits, since $0.5 \times 0.81 = 0.40$. Figure \ref{fig:region} shows the same conclusion in the two-dimensional scan of the $(\kappa, \sigma_m)$ plane, where the multiplicity region sits at low dispersion, the data-fitting region at higher dispersion, and the two are adjacent and disjoint. The same scan at $\omega = 0.15$ and $\omega = 0.50$ gives the same answer: at low $\omega$ multiplicity is widespread but nothing in it fits the moments, and at high $\omega$ multiplicity disappears entirely.
...
Take the Gift Aid rate as the central case, since it is the smaller and better-identified of the two. A 25 percent rebate raises equilibrium participation from 37.1 to 83.3 percent at a fiscal cost of 4.1 percent of mean labour income, roughly a fifth of what the work subsidy costs, with the sign reversed and with both groups strictly interior at 76 and 91 percent. The French 66 percent deduction, a much larger rebate, drives participation to 99.7 percent at a cost of 12.7 percent of mean income. That figure should be read as an upper bound rather than a forecast: at a rebate that large the model saturates, the higher-education group goes to a corner, and the exercise stops being informative about levels while remaining informative about direction. The mechanism in both cases is the social feedback running the desirable way: each induced entry raises the participation rate, which raises every other household's belonging return, which induces further entries. The same complementarity that amplified the work subsidy's eight-point direct effect into a twenty-nine-point collapse turns a modest rebate into a large inflow. Implementing the rebate inside the budget constraint, rather than as a multiplier on the social payoff, if anything strengthens it, because the rebate also relaxes the participant's consumption and the income effect reinforces the social one.

\subsubsection*{Take-up incidence}

Credits of this kind are not claimed evenly. Itemisation, salience and the paperwork of claiming all favour better-off households, so the headline numbers assume a take-up rate the instrument would not achieve. I model this with a take-up ratio $\tau$: the lower-education group's effective rebate is $\tau \rho$ while the higher group receives the full $\rho$, and the fiscal cost counts only credits actually claimed. Table \ref{tab:takeup} runs $\tau \in \{1, 0.5, 0.25\}$.

\begin{table}[t]
\centering
\caption{Participation credit under imperfect take-up by the lower-education group. The ratio of high to low participation falls from 1.79 at baseline in every scenario: the credit is equalising even when the group that claims least is the one that starts lowest.}
\label{tab:takeup}
\begin{tabular}{lcccc}
\hline
 & participation & low & high & cost (\% of mean income) \\
\hline
baseline & 0.371 & 0.265 & 0.476 & --- \\
Gift Aid 25\%, full take-up & 0.833 & 0.760 & 0.907 & 4.1 \\
Gift Aid 25\%, $\tau = 0.5$ & 0.823 & 0.741 & 0.906 & 3.2 \\
Gift Aid 25\%, $\tau = 0.25$ & 0.732 & 0.584 & 0.880 & 2.6 \\
France 66\%, full take-up & 0.997 & 0.993 & 1.000 & 12.7 \\
France 66\%, $\tau = 0.5$ & 0.941 & 0.881 & 1.000 & 9.5 \\
France 66\%, $\tau = 0.25$ & 0.883 & 0.765 & 1.000 & 8.1 \\
\hline
\end{tabular}
\end{table}

Two things come out of the exercise, and the first was not what I expected. Imperfect take-up does not make the credit regressive. The ratio of higher- to lower-group participation is 1.79 at baseline and falls in every scenario, to 1.01 when take-up is complete and still to 1.31 and 1.51 when the lower group claims at only a quarter of the higher group's rate. The credit equalises participation even when the households who claim it least are the ones who start furthest behind, because the lower group has the most room to move and the complementarity carries part of the adjustment for it. Poor take-up makes the instrument less equalising, not regressive.

Second, take-up disciplines the level without touching the sign. Gift Aid's aggregate falls from 83 to 73 percent as take-up drops to a quarter, and its cost falls from 4.1 to 2.6 percent of mean income, so the policy stays cheap relative to the work subsidy throughout. The French rate is the exception that proves the earlier caution: because take-up throttles only the lower group while the higher group still receives the full rebate, the higher group sits at a corner whatever $\tau$ is, which is why the 66 percent case belongs in the paper as a bound and not as an estimate.
...
\hline
baseline & 0.371 & --- & --- \\
work subsidy (20\%) & 0.081 & $+7.2\%$ & $-2.2\%$ \\
empowerment & 0.507 & $+2.2\%$ & $+5.9\%$ \\
participation credit (66\%) & 0.997 & $-6.7\%$ & $+12.5\%$ \\
\hline
\end{tabular}
\end{table}

The two ledgers tell opposite stories about the same fiscal choice. The work subsidy raises material GDP by 7.2 percent and lowers GDP-B by 2.2 percent: it manufactures measured output by dismantling the fabric. The participation credit does the reverse, lowering material GDP by 6.7 percent, because participation takes time that would otherwise be worked, while raising GDP-B by 12.5 percent. Empowerment raises both. The two fiscal policies are near mirror images across the two ledgers (Figure \ref{fig:gdpb}), which is the beyond-GDP programme's point stated inside a structural model: the choice between subsidising hours and rewarding participation is not a matter of degree on one axis but of direction on two.

\begin{figure}[t]
\centering
\includegraphics[width=0.72\textwidth]{figures/sa_gdpb.png}
\caption{Material GDP (consumption only) against GDP-B (consumption plus the social fabric) for the baseline and the three policies. The work subsidy is the only policy that raises GDP while lowering GDP-B; the participation credit is its mirror image.}
\label{fig:gdpb}
\end{figure}

\section{Conclusion}

The paper's central result is a bound. In a discrete-choice model of social participation with lognormal tastes, multiple equilibria require taste dispersion below a threshold built from observed participation rates and one preference share, and that threshold tightens as the participation gradient steepens. Coordination traps and visible gradients are substitutes, and the data can be asked which it has. Asked of France, it answers gradients: the economy sits about ten percent outside the coordination region, conservatively, since the bound overstates the true slope. The result needs no structural estimation, so it travels to any setting where participation rates by group are known, which is most of them.

Around that bound the paper gives the SAGE agency dimension a structural home on the social side of the model. Agency sets who can afford to participate, through wealth rather than through the price of time, which is why participation rises with income in the model as in the data, and which supplies the dispersed tastes the bound then rules on. Policy lives at the edge the bound locates. The social complementarity amplifies a financed work subsidy's eight-point direct effect into a twenty-nine-point collapse of participation; it amplifies an empowerment policy into a participation boom of fourteen points without a fiscal instrument; and it amplifies a participation tax credit into a still larger boom at a lower fiscal cost than the work subsidy, with the sign reversed. The same calibrated complementarity does all three pieces of arithmetic. A recognisable real-world policy that pays direct attention to the social margin therefore outperforms the work-side policy by orders of magnitude on its own terms, and the contrast is invisible to single-index evaluation.
