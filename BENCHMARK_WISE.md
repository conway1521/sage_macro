...
## Addendum, 2026-09-08: the participation object fixes the inversion

The S+A paper's cohesion object is participation time rather than all non-work time. On the corrected core (logit participation, theta 0.005, rescaled asset grid, calibration (10.00, 0.495)) the model's participation rate against the WISE Solidarity Index gives Spearman +0.94, +0.49, +0.46 across all six countries in 2007, 2017 and 2018, and +0.80, +1.00, +0.95 across the OECD four, where the time-based object above gave -0.15 to -0.80. It beats the calibrated belonging and agency inputs alone in every cell, so it is not inheriting the correlation from what went in, and the country levels are separated by at least 0.02 against a numerical noise near 0.002, so the ordering is real. Six countries, so suggestive rather than established; but the direction of the earlier conclusion, that the cohesion object needed rethinking, was right, and the rethinking the S+A paper did is what fixes it. Script `SAGE_Bewley/scripts/wise_participation_logit.jl`.
